...
/* handle http verb and body */
static ngx_int_t ngx_dashll_access_handler(ngx_http_request_t *r)
{
  ngx_int_t rc;
  ngx_dashll_ctx_t *ctx;

  ngx_log_error(NGX_LOG_EMERG, r->connection->log, 0, "ngx_dashll_access_handler method %d",
                r->method);
  if (r->method != NGX_HTTP_POST) {
    return NGX_DECLINED;
  }

  /* xxx check headers such as request url and content-type here */
  ...
  /* should only get called once? is this where unbuffered should be called? */
  /* https://nginx-devel.nginx.narkive.com/MUAyCD53/ngx-http-read-client-request-body-and-access-phase-handler */
  ...
  return NGX_DECLINED;
  /*return NGX_DONE; - this causes request to hang. why? */
}
